...
export const LeadDetailView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 bg-red-600 text-white rounded-xl font-bold shadow-lg shadow-red-600/20 hover:bg-red-700 transition-all text-sm ${lead.status === 'Perdido' ? 'hidden' : ''}`}>
        ...
        <div className="lg:col-span-3 card-glass p-6 flex flex-col">
          ...
          <div className="grid grid-cols-2 gap-3 flex-1">
            ...
            <button onClick={() => setShowAgendar(true)}
              ...
              className="flex flex-col items-center justify-center gap-2 p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-transparent hover:border-amber-200 hover:bg-amber-50 dark:hover:bg-amber-900/10 text-slate-600 dark:text-slate-300 hover:text-amber-500 transition-all group disabled:opacity-30 disabled:cursor-not-allowed">
          ...
          {/* Desqualificar — oculto se já perdido */}
          {lead.status !== 'Perdido' && (
          <button onClick={() => setShowDeleteModal(true)}
      ...
      {/* ── Banner Lead Perdido ── */}
      {lead.status === 'Perdido' && (
        <div className="card-glass border-l-4 border-l-red-500 p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <p className="text-sm font-black text-red-700 dark:text-red-400 uppercase tracking-wide">Lead Desqualificado</p>
                <span className="text-[10px] font-bold bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 px-2 py-0.5 rounded-full uppercase tracking-widest">Perdido</span>
              </div>
              {(() => {
                const motivoEvent = lead.history.find(h => h.content?.startsWith('Motivo da desqualificação:'));
                return motivoEvent
                  ? <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                      <span className="font-semibold">Motivo:</span> {motivoEvent.content.replace('Motivo da desqualificação:', '').trim()}
                    </p>
                  : <p className="text-xs text-slate-400 mt-1 italic">Motivo não registrado.</p>;
              })()}
            </div>
            <button
              onClick={() => {
                const actorName = user?.user_metadata?.full_name || user?.email?.split('@')[0] || 'Consultor';
                updateLeadStatus(lead.id, 'Novo Lead', actorName);
                addNote(lead.id, 'Lead reativado pelo consultor.', 'consultant', actorName);
              }}
              className="px-3 py-2 text-xs font-bold text-red-600 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 border border-red-200 dark:border-red-800 rounded-xl transition-all whitespace-nowrap shrink-0"
            >
              Reativar Lead
            </button>
          </div>
        ...
        {showDeleteModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <p className="text-slate-500 text-center text-sm mb-4 leading-relaxed">O lead será movido para Perdidos e sairá do funil ativo.</p>
              <div className="mb-6">
                ...
                <select
                  ...
                  className="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm dark:text-white focus:outline-none focus:ring-2 focus:ring-red-600/20 mb-2"
                >
                  <option value="">Selecionar motivo...</option>
                  <option value="Preço alto">Preço alto</option>
                  <option value="Distância">Distância</option>
                  <option value="Escolheu outra escola">Escolheu outra escola</option>
                  <option value="Criança fora da faixa etária">Criança fora da faixa etária</option>
                  <option value="Sem retorno do responsável">Sem retorno do responsável</option>
                ...
                {lostReason === 'Outro' && (
                  <input
                    ...
                    className="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm dark:text-white focus:outline-none focus:ring-2 focus:ring-red-600/20"
                    onChange={e => setLostReason(e.target.value === '' ? 'Outro' : e.target.value)}
                  />
                )}
              </div>
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => { setShowDeleteModal(false); setLostReason(''); }}
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    addNote(lead.id, `Motivo da desqualificação: ${lostReason}`, 'consultant', actorName);
                    setShowDeleteModal(false);
                    setLostReason('');
                    setCurrentView(ViewState.LEADS);
                  }}
                  className="py-3 px-4 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 transition-all disabled:opacity-40 disabled:cursor-not-allowed">
                  Confirmar
                </button>
              </div>
            </motion.div>
